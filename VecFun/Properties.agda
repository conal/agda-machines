...
module _ where
  ...
  open ≡ using (_≡_; cong; cong₂; module ≡-Reasoning)
  open ≡-Reasoning
  ...
  delay∷ : ∀ {a₀ a : A} {as : Vec A n} → ⟦ delay a₀ ⟧ₛ (a ∷ as) ≡ a₀ ∷ ⟦ delay a ⟧ₛ as
  delay∷ = init∷

  init∘scanl′ : ∀ {f : B → A → B} {e : B} (as : Vec A n)
              → init (scanl′ f e as) ≡ ⟦ scanl f e ⟧ₛ as
  init∘scanl′ [] = ≡.refl
  init∘scanl′ {f = f}{e} (a ∷ as) =
    ...
    ≡⟨ cong (e ∷_) (init∘scanl′ as) ⟩
      e ∷ ⟦ scanl f (f e a) ⟧ₛ as
    ∎

  scanl∷ʳ : ∀ {f : B → A → B} {e : B} (as : Vec A n)
           → scanl′ f e as ≡ ⟦ scanl f e ⟧ₛ as ∷ʳ foldl _ f e as
  scanl∷ʳ [] = ≡.refl
  scanl∷ʳ {f = f}{e} (a ∷ as) =
    begin
      scanl′ f e (a ∷ as)
    ≡⟨⟩
      e ∷ scanl′ f (f e a) as
    ≡⟨ cong (e ∷_) (scanl∷ʳ as) ⟩
      e ∷ (⟦ scanl f (f e a) ⟧ₛ as ∷ʳ foldl _ f e (a ∷ as))
    ≡⟨⟩
      (e ∷ ⟦ scanl f (f e a) ⟧ₛ as) ∷ʳ foldl _ f e (a ∷ as)
    ≡⟨⟩
      ⟦ scanl f e ⟧ₛ (a ∷ as) ∷ʳ foldl _ f e (a ∷ as)
    ∎
