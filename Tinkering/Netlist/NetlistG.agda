...
private variable a b c d i o j k : ℕ

infixr 5 _∷_
data Vec′ (F : ℕ → ℕ → ℕ → Set) (i : ℕ) : ℕ → Set where
  [] : Vec′ F i 0
  _∷_ : F k i j → Vec′ F i k → Vec′ F i (j + k)

-- In Vec′ F i k, we've *added* k.
...
Inst : ℕ → ℕ → ℕ → Set
Inst k i b = ∃ λ a → (a p.⇨ b) × (k + i r.⇨ a)

⟦_⟧ⁱ : Inst k i b → k + i b.⇨ b
⟦ _ , p , r ⟧ⁱ = p.⟦ p ⟧ F.∘ r.⟦ r ⟧

Netlist : ℕ → ℕ → Set
Netlist = Vec′ Inst

⟦_⟧ⁿ : Netlist i k → i b.⇨ k + i
⟦ [] ⟧ⁿ = F.id
⟦_⟧ⁿ {i = i} (_∷_ {k = k}{j = j} inst nl) =
  let x = ⟦ nl ⟧ⁿ ; y = ⟦_⟧ⁱ {k = k}{i = i} inst in
     b.assocˡ {j}{k}{i} b.∘ (y b.△ b.id) b.∘ x
...
infix 0 _⇨_
_⇨_ : ℕ → ℕ → Set
i ⇨ o = ∃ λ k → Netlist i k × (k + i r.⇨ o)

route : a r.⇨ b → a ⇨ b
route {a = a} a⇨ᵣb = 0 , [] , a⇨ᵣb
...
-- 
-- TODO: prove route is a cartesian functor, so all such alternatives are
-- equivalent.

prim : a p.⇨ b → a ⇨ b
prim {a}{b} a⇨ₚb =
 b , subst (Netlist a) (+-identityʳ b) ((a , a⇨ₚb , r.unitorᵉˡ) ∷ []) , r.exl
...
_∘_ {c = c} (kg , [] , kg⇨ᵣc) (kf , nf , kf⇨ᵣb) =
  kf , nf , kg⇨ᵣc r.∘ kf⇨ᵣb

_∘_ {b = b} {c = c} {a = a}
     (.(j + kg) , _∷_ {k = kg}{j = j} (i , i⇨ₚj , kg⇨ᵣi) ng , j+kg⇨ᵣc) f =
  let X@(z , n , q) = (kg , ng , kg⇨ᵣi) ∘ f in
...
_∘_ {b = b} {c = c} {a = a}
     (.(j + kg) , _∷_ {k = kg}{j = j} (i , i⇨ₚj , kg⇨ᵣi) ng , j+kg⇨ᵣc) f =
  let X@(z , n , q) = (kg , ng , {!!}) ∘ f in

    {!!} , _∷_ {k = {!!}} {j = {!!}} (i , i⇨ₚj , {!kg!}) n , {!j!}

    -- j + i , _∷_ {k = i} {j = j} (i , i⇨ₚj , λ z → z) {!n!} , {!? r.∘ r.second ?!}


-- Inst k b = ∃ λ a → (a p.⇨ b) × (k r.⇨ a)

  -- _∷_ : F k j → Vec′ F i k → Vec′ F i (j + k)


-- _∘_ {c = c} (.(_ + _) , inst ∷ ng , kg⇨ᵣc) (kf , nf , kf⇨ᵣb) =
--   let h = ? ∘ 
--   {!!} , {!!} , {!!}

-- _∘_ {c = c} (kg , ng , kg⇨ᵣc) (kf , nf , kf⇨ᵣb) =
--   kg + kf , {!!} , {!!}
