...
module _ {i : ℕ} where
  ...
  Inst : ℕ → ℕ → Set
  Inst k b = ∃ λ a → (a p.⇨ b) × (k + i r.⇨ a)

  ⟦_⟧ⁱ : Inst k b → k + i b.⇨ b
  ⟦ j , j⇨ₚb , k+i⇨ᵣj ⟧ⁱ = p.⟦ j⇨ₚb ⟧ b.∘ r.⟦ k+i⇨ᵣj ⟧

  Netlist : ℕ → Set
  Netlist = Vec′ Inst

  ⟦_⟧ⁿ : Netlist k → i b.⇨ k + i
  ⟦ [] ⟧ⁿ = b.unitorⁱˡ
  ⟦ _∷_ {k = k}{j = j} inst nl ⟧ⁿ =
     b.assocˡ {j}{k}{i} b.∘ Q ⟦ inst ⟧ⁱ b.∘ ⟦ nl ⟧ⁿ

  -- A netlist with k outputs and result size a
  Src : ℕ → ℕ → Set
  Src k b = Netlist k × (k + i r.⇨ b)
...
route : a r.⇨ b → a ⇨ b
route {a} a⇨ᵣb = 0 , [] , r.unitorᵉˡ r.∘ a⇨ᵣb
